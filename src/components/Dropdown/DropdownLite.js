...
import {
	...
	isVisible,
	noop,
} from 'bootstrap/js/dist/util/index.js'
...
class DropdownLite extends BaseComponent {
	...
	static clearMenus(event) {
		...
		for (const toggle of openToggles) {
			const context = DropdownLite.getInstance(toggle)
...
function dataApiKeydownHandler(event) {
	...
	const instance = DropdownLite.getOrCreateInstance(getToggleButton)
...
EventHandler.on(document, EVENT_CLICK_DATA_API, DropdownLite.clearMenus)
EventHandler.on(document, EVENT_KEYUP_DATA_API, DropdownLite.clearMenus)
EventHandler.on(
	document,
	...
	function (event) {
		event.preventDefault()
		DropdownLite.getOrCreateInstance(this).toggle()
	},
)
